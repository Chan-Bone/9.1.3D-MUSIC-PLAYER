...
module Genre
  POP, CLASSIC, JAZZ, ROCK, DIGITAL, FUSION = *1..6
end

Genre_names = ['Null', 'Pop', 'Classic', 'Jazz', 'Rock', 'Digital', 'Fusion'].freeze
...
# complete the missing code:
	def initialize (artist, title, recording_type, genre, tracks)
		@artist = artist
    @title = title
...
def read_track(music_file)
	contents = [music_file.gets().to_s().chomp, music_file.gets().to_s().chomp, music_file.gets().to_s().chomp]
...
def read_album(music_file)
  ...
  # complete the missing code
	...
	album = Album.new(album_artist, album_title, recording_type, album_genre, tracks)
	return album
end
